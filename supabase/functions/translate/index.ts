...
function replaceStandalonePronouns(text: string, pattern: RegExp, replacement: string): string {
  return text.replace(pattern, (match, _capture, offset: number, full: string) => {
...
function normalizePronouns(translation: string, targetLang: string, enabled: boolean): string {
  if (!enabled) return translation;
  if (targetLang === 'en') {
    return replaceStandalonePronouns(translation, /\b(he|she)\b/gi, 'he/she');
  }
  if (targetLang === 'es') {
    return replaceStandalonePronouns(translation, /\b(él|ella)\b/gi, 'él/ella');
  }
  return translation;
}
...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request
    const { source_lang, target_lang, text, neutralize_pronouns }: TranslationRequest = await req.json();
    ...
    if (cached && !cacheError) {
      const response: TranslationResponse = {
        term,
        term_normalized,
        translation: normalizePronouns(cached.translation, target_lang, neutralize_pronouns !== false),
        from_cache: true,
      };
    ...
    translation = normalizePronouns(translation, target_lang, neutralize_pronouns !== false);
