-- Extend lingoleaf.books table with catalog metadata for large-scale ingestion (e.g., Gutendex)
...
CREATE INDEX IF NOT EXISTS idx_books_source ON lingoleaf.books(source);
CREATE INDEX IF NOT EXISTS idx_books_source_lang ON lingoleaf.books(source_lang);
CREATE INDEX IF NOT EXISTS idx_books_author ON lingoleaf.books(author);

-- Array filters (subjects/languages)
CREATE INDEX IF NOT EXISTS idx_books_subjects_gin ON lingoleaf.books USING GIN (subjects);
CREATE INDEX IF NOT EXISTS idx_books_languages_gin ON lingoleaf.books USING GIN (languages);
