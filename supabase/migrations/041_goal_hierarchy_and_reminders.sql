-- [2026-03-04] [DB] Add primary-goal hierarchy + local reminder preferences to lingoleaf.user_settings

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'lingoleaf' AND table_name = 'user_settings' AND column_name = 'primary_goal'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'lingoleaf' AND table_name = 'user_settings' AND column_name = 'goal_priority'
  ) THEN
    ALTER TABLE lingoleaf.user_settings
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'lingoleaf' AND table_name = 'user_settings' AND column_name = 'daily_goal_reminder_enabled'
  ) THEN
    ALTER TABLE lingoleaf.user_settings
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'lingoleaf' AND table_name = 'user_settings' AND column_name = 'daily_goal_reminder_hour'
  ) THEN
    ALTER TABLE lingoleaf.user_settings
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'lingoleaf' AND table_name = 'user_settings' AND column_name = 'daily_goal_reminder_minute'
  ) THEN
    ALTER TABLE lingoleaf.user_settings
