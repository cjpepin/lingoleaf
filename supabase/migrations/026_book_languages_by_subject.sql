-- Language filter: when subject(s) are selected, only return languages that have books in those subjects
CREATE OR REPLACE FUNCTION get_distinct_book_languages(p_subjects text[] DEFAULT NULL)
