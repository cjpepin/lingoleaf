...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'user_settings' AND column_name = 'daily_reading_goal_minutes'
  ) THEN
...
CREATE INDEX IF NOT EXISTS idx_vocab_reviews_user_reviewed
  ON vocab_reviews(user_id, reviewed_at DESC);

ALTER TABLE reading_sessions ENABLE ROW LEVEL SECURITY;
ALTER TABLE vocab_reviews ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can read their own reading_sessions" ON reading_sessions;
CREATE POLICY "Users can read their own reading_sessions"
  ON reading_sessions FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can insert their own reading_sessions" ON reading_sessions;
CREATE POLICY "Users can insert their own reading_sessions"
  ON reading_sessions FOR INSERT
  TO authenticated
  WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can update their own reading_sessions" ON reading_sessions;
...
DROP POLICY IF EXISTS "Users can delete their own reading_sessions" ON reading_sessions;
CREATE POLICY "Users can delete their own reading_sessions"
  ON reading_sessions FOR DELETE
  TO authenticated
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can read their own vocab_reviews" ON vocab_reviews;
CREATE POLICY "Users can read their own vocab_reviews"
  ON vocab_reviews FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can insert their own vocab_reviews" ON vocab_reviews;
CREATE POLICY "Users can insert their own vocab_reviews"
  ON vocab_reviews FOR INSERT
  TO authenticated
  WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can update their own vocab_reviews" ON vocab_reviews;
CREATE POLICY "Users can update their own vocab_reviews"
  ON vocab_reviews FOR UPDATE
  TO authenticated
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can delete their own vocab_reviews" ON vocab_reviews;
CREATE POLICY "Users can delete their own vocab_reviews"
  ON vocab_reviews FOR DELETE
  TO authenticated
  USING (user_id = auth.uid());

COMMENT ON COLUMN user_settings.daily_reading_goal_minutes IS 'Daily reading goal in minutes. Allowed values: 5, 10, 15.';
COMMENT ON TABLE reading_sessions IS 'Per-reading session records used for progress and goal tracking.';
COMMENT ON TABLE vocab_reviews IS 'Review event log for study words used in progress aggregates.';
