-- Expand daily goal targets for reading/saved/learned in user_settings

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'user_settings' AND column_name = 'daily_words_saved_goal'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'user_settings' AND column_name = 'daily_words_learned_goal'
  ) THEN
    ALTER TABLE user_settings
