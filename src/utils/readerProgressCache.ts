...
export async function setCachedLastCfi(
  ...
  try {
    let existing: Partial<CachedCfi> | null = null;
    if (preservePosition) {
      ...
      existing = rawExisting ? (JSON.parse(rawExisting) as Partial<CachedCfi>) : null;
    }

    const payload: CachedCfi = {
      cfi,
      updatedAt: nextUpdatedAt,
      locationIndex0: preservePosition &&
        typeof existing?.locationIndex0 === 'number' &&
        Number.isFinite(existing.locationIndex0)
          ? existing.locationIndex0
          : undefined,
      totalLocations: preservePosition &&
        typeof existing?.totalLocations === 'number' &&
        Number.isFinite(existing.totalLocations)
...
export async function setCachedTotalLocations(
  userId: string | null,
  bookId: string,
  totalLocations: number,
  cfi?: string
): Promise<void> {
  if (!Number.isFinite(totalLocations) || totalLocations <= 0) return;
  const key = keyFor(userId, bookId);
  try {
    const rawExisting = await AsyncStorage.getItem(key);
    const existing = rawExisting ? (JSON.parse(rawExisting) as Partial<CachedCfi>) : null;
    const resolvedCfi =
      typeof cfi === 'string' && cfi.length > 0
        ? cfi
        : (typeof existing?.cfi === 'string' && existing.cfi.length > 0 ? existing.cfi : '');
    ...
    const payload: CachedCfi = {
      ...
      locationIndex0:
        typeof existing?.locationIndex0 === 'number' && Number.isFinite(existing.locationIndex0)
          ? Math.floor(existing.locationIndex0)
          : undefined,
      totalLocations: Math.floor(totalLocations),
    };
    await AsyncStorage.setItem(key, JSON.stringify(payload));
  } catch {
    // best-effort
