...
import { isUserSoftDeleted, reactivateUserAccount } from '@/supabase/queries';
...
function isEmailNotConfirmedError(error: unknown): boolean {
  ...
  if (message === 'email_not_confirmed') return true;
  if (message.includes('email_not_confirmed')) return true;
...
function isEmailDeliveryConfigError(error: unknown): boolean {
  const message = String((error as any)?.message ?? '').toLowerCase();
  if (!message) return false;
  if (message.includes('error sending confirmation email')) return true;
  if (message.includes('sender identity')) return true;
  if (message.includes('smtp')) return true;
  return false;
}
...
export default function AuthScreen() {
  ...
  const [lastConfirmResendAt, setLastConfirmResendAt] = useState<number>(0);
  const { user, signIn, signUp, signOut: signOutStore, isGuest } = useAuthStore();
  ...
  const handlePostAuthMigration = async (
    ...
    if (!shouldMigrate) {
      logger.info('Skipping migration: auth action is not account creation');
      return true;
    }
    ...
    const result = await migrateUserDataIfNeeded(fromUserId, toUserId, session.access_token);
    if (!result.ok) {
      Alert.alert(t('auth.migrationFailedTitle'), result.error || t('auth.migrationFailedBody'));
      return false;
    }
    return true;
  };
  ...
  const handleAppleSignIn = async (): Promise<void> => {
    ...
    track(effectiveIsSignUp ? 'sign_up_started' : 'login_started', {
      method: 'apple',
      source: mode === 'upgrade' ? 'upgrade_paywall' : 'auth_screen',
    });
    ...
    try {
      ...
      const pendingFromUserId = await getPendingGuestMigrationUserId();
      const migrationFromUserId = pendingFromUserId ?? fromUserId;
      const shouldMigrateNow = Boolean(migrationFromUserId) && (effectiveIsSignUp || Boolean(pendingFromUserId));
      const migrationOk = await handlePostAuthMigration(migrationFromUserId, data?.session ?? null, shouldMigrateNow);
      if (migrationOk && pendingFromUserId) {
        await clearPendingGuestMigrationUserId();
      }
      const canContinue = await maybeReactivateDeletedAccount(data?.session?.user?.id);
      if (!canContinue) return;
      track(effectiveIsSignUp ? 'sign_up_completed' : 'login_completed', {
        method: 'apple',
        source: mode === 'upgrade' ? 'upgrade_paywall' : 'auth_screen',
      });
      if (mode === 'upgrade') {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (session) {
        ...
        const migrationFromUserId = pendingFromUserId ?? fromUserId;
        const shouldMigrateNow = Boolean(migrationFromUserId) && (effectiveIsSignUp || Boolean(pendingFromUserId));
        const migrationOk = await handlePostAuthMigration(migrationFromUserId, session, shouldMigrateNow);
        if (migrationOk && pendingFromUserId) {
          await clearPendingGuestMigrationUserId();
        }
      }
      const canContinue = await maybeReactivateDeletedAccount(session?.user?.id);
      if (!canContinue) return;
      track(effectiveIsSignUp ? 'sign_up_completed' : 'login_completed', {
        method: 'email',
        source: mode === 'upgrade' ? 'upgrade_paywall' : 'auth_screen',
      ...
      if (error.message === 'EMAIL_CONFIRMATION_REQUIRED') {
        if (effectiveIsSignUp && fromUserId) {
          await setPendingGuestMigrationUserId(fromUserId);
        }
      ...
      if (isEmailNotConfirmedError(error)) {
        const now = Date.now();
        const elapsed = now - lastConfirmResendAt;
        const cooldownMs = 60_000;
        const remainingSeconds = Math.ceil((cooldownMs - elapsed) / 1000);

        const resend = async () => {
          const resendNow = Date.now();
          if (resendNow - lastConfirmResendAt < cooldownMs) {
            const waitSeconds = Math.ceil((cooldownMs - (resendNow - lastConfirmResendAt)) / 1000);
            setSnackbar({
              visible: true,
              message: t('auth.resendCooldown', { seconds: Math.max(waitSeconds, 1) }),
              type: 'info',
            });
            return;
          }
          ...
          setLastConfirmResendAt(Date.now());
          setSnackbar({
            visible: true,
        ...
        Alert.alert(
          ...
          t('auth.emailNotConfirmedBody', { email }),
          [
            { text: t('common.cancel'), style: 'cancel' },
            {
              text: elapsed >= cooldownMs
                ? t('auth.resendEmail')
                : t('auth.resendCooldownButton', { seconds: Math.max(remainingSeconds, 1) }),
              onPress: () => { void resend(); },
            },
          ]
        );
      ...
      if (isEmailDeliveryConfigError(error)) {
        setSnackbar({
          visible: true,
          message: t('auth.emailDeliveryUnavailable'),
          type: 'error',
        });
        return;
      }
  ...
  const handleForgotPassword = async () => {
    if (!email) {
      setSnackbar({ visible: true, message: t('auth.enterEmailForReset'), type: 'error' });
      return;
    }
    try {
      ...
      if (error) throw error;
      setSnackbar({ visible: true, message: t('auth.resetSent'), type: 'success' });
    } catch (error) {
      logger.error('Failed to send password reset email', error);
      setSnackbar({ visible: true, message: t('auth.resetFailed'), type: 'error' });
    } finally {
      setSendingReset(false);
    }
  };

  const maybeReactivateDeletedAccount = async (userId?: string): Promise<boolean> => {
    if (!userId) return true;
    const softDeleted = await isUserSoftDeleted(userId);
    if (!softDeleted) return true;

    const shouldReactivate = await new Promise<boolean>((resolve) => {
      Alert.alert(
        t('auth.reactivateDeletedTitle'),
        t('auth.reactivateDeletedBody'),
        [
          {
            text: t('auth.keepDeleted'),
            style: 'cancel',
            onPress: () => resolve(false),
          },
          {
            text: t('auth.reactivateAccount'),
    ...
    if (!shouldReactivate) {
      ...
      setSnackbar({
        visible: true,
        message: t('auth.reactivateDeclinedSignedOut'),
        type: 'info',
      });
      return false;
    }
    ...
    try {
      ...
      setSnackbar({
        visible: true,
        message: t('auth.reactivateSuccess'),
        type: 'success',
      });
      return true;
    } catch (error) {
      ...
      setSnackbar({
        visible: true,
        message: t('auth.reactivateFailed'),
        type: 'error',
      });
      return false;
    }
  ...
  return (
    ...
    >
      <View style={styles.content}>
        <Text style={styles.title}>{mode === 'upgrade' ? t('auth.signUp') : t('app.title')}</Text>
