...
import { View, Text, StyleSheet, Alert, Pressable, Animated, ActivityIndicator, AppState } from 'react-native';
...
import { getCachedLastCfi, setCachedLastCfi, setCachedLastPosition, setCachedTotalLocations } from '@/utils/readerProgressCache';
...
import { emitDailyGoalReadingEvent } from '@/hooks/dailyGoalEvents';
import {
...
function isSameCfiLocation(a: string | null | undefined, b: string | null | undefined): boolean {
  if (!a || !b) return false;
  return a === b || a.startsWith(b) || b.startsWith(a);
}

function resolvePersistableReaderCfi(params: {
  currentCfi: string | null;
  fallbackCfi: string | null;
  initialLocation?: string;
  resumeConfirmed: boolean;
}): string | null {
  const { currentCfi, fallbackCfi, initialLocation, resumeConfirmed } = params;
  if (currentCfi && (!initialLocation || resumeConfirmed || isSameCfiLocation(currentCfi, initialLocation))) {
    return currentCfi;
  }
  return fallbackCfi;
}
...
export default function ReaderScreen() {
  ...
  const { goalMinutes: dailyGoalMinutes, minutesToday: dailyMinutesFromHistory } = useDailyGoal();
  const currentPage = useReaderStore((s) => s.progressByBook[bookId]?.currentPage ?? 0);
  const totalPages = useReaderStore((s) => s.progressByBook[bookId]?.totalPages ?? 0);
  const pageLoading = useReaderStore((s) => s.progressByBook[bookId]?.pageLoading ?? true);
  const chapterPage = useReaderStore((s) => s.progressByBook[bookId]?.chapterPage ?? null);
  const chapterTotal = useReaderStore((s) => s.progressByBook[bookId]?.chapterTotal ?? null);
  const setCurrentPage = useReaderStore((s) => s.setCurrentPage);
  const setTotalPages = useReaderStore((s) => s.setTotalPages);
  const setPageLoading = useReaderStore((s) => s.setPageLoading);
  const setChapterLeftPct = useReaderStore((s) => s.setChapterLeftPct);
  const setChapterDisplayed = useReaderStore((s) => s.setChapterDisplayed);
  const resetProgress = useReaderStore((s) => s.resetProgress);
  ...
  const [sessionMinutesLive, setSessionMinutesLive] = useState(0);
  const [hasResolvedCurrentPage, setHasResolvedCurrentPage] = useState(false);
  ...
  const sortedVocabLists = useMemo(() => sortStudyListsByRecentUpdate(vocabLists), [vocabLists]);
  const lockedVocabListIds = useMemo(
    () => getLockedStudyListIds(sortedVocabLists, isPremium, FREE_STUDY_LIST_CAP),
    [isPremium, sortedVocabLists]
  );
  const accessibleVocabLists = useMemo(
    () => sortedVocabLists.filter((list) => !lockedVocabListIds.has(list.id)),
    [lockedVocabListIds, sortedVocabLists]
  );
  const readerMinutesToday = useMemo(
    () => Math.max(0, dailyMinutesFromHistory + sessionMinutesLive),
    [dailyMinutesFromHistory, sessionMinutesLive]
  );
  ...
  const latestPersistableCfiRef = useRef<string | null>(null);
  ...
  const cachedBookTotalPagesRef = useRef(0);
  const readyBookIdRef = useRef<string | null>(null);
  const renderedBookIdRef = useRef(bookId);
  ...
  const resumeConfirmedRef = useRef(false);
  const activeSegmentStartRef = useRef<number | null>(null);
  const activeDurationMsRef = useRef(0);
  const screenActiveDurationMsRef = useRef(0);
  const appStateRef = useRef(AppState.currentState);
  const [resumeConfirmed, setResumeConfirmed] = useState(false);

  if (renderedBookIdRef.current !== bookId) {
    renderedBookIdRef.current = bookId;
    readyBookIdRef.current = null;
    cachedBookTotalPagesRef.current = 0;
  }
  ...
  useEffect(() => {
    ...
    if (resumeFallbackTimerRef.current) clearTimeout(resumeFallbackTimerRef.current);
    ...
    latestPersistableCfiRef.current = null;
  ...
  useEffect(() => {
    if (!shouldFinishPageLoading({ hasResolvedCurrentPage, totalPages, pageLoading })) return;
    setPageLoading(bookId, false);
  }, [bookId, hasResolvedCurrentPage, pageLoading, setPageLoading, totalPages]);

  useEffect(() => {
    readerReadyRef.current = readerReady;
  }, [readerReady]);
  ...
  useEffect(() => {
    currentLocationCfiRef.current = currentLocation?.start?.cfi ?? null;
  }, [currentLocation?.start?.cfi]);

  const flushReadingProgress = useCallback(async (cfi: string) => {
    if (!cfi) return;
    latestPersistableCfiRef.current = cfi;
    await setCachedLastCfi(user?.id ?? null, bookId, cfi);
  ...
  const flushCurrentReadingProgress = useCallback(async () => {
    const cfi = resolvePersistableReaderCfi({
      currentCfi: currentLocationCfiRef.current,
      fallbackCfi: latestPersistableCfiRef.current,
      initialLocation,
      resumeConfirmed: resumeConfirmedRef.current,
    });
    if (!cfi) return;
    ...
    await flushReadingProgress(cfi);
  }, [flushReadingProgress, initialLocation]);

  const getSessionMinutesLive = useCallback((nowMs: number = Date.now()) => {
    const startedAt = activeSegmentStartRef.current;
    const activeMs = activeDurationMsRef.current + (startedAt != null ? Math.max(0, nowMs - startedAt) : 0);
    return Math.max(0, Math.floor(activeMs / MS_PER_MINUTE));
  }, []);

  const pauseActiveSession = useCallback((nowMs: number = Date.now()) => {
    const startedAt = activeSegmentStartRef.current;
    if (startedAt == null) return;
    const elapsedMs = Math.max(0, nowMs - startedAt);
    activeDurationMsRef.current += elapsedMs;
    screenActiveDurationMsRef.current += elapsedMs;
    activeSegmentStartRef.current = null;
    setSessionMinutesLive(getSessionMinutesLive(nowMs));
  }, [getSessionMinutesLive]);

  const resumeActiveSession = useCallback((nowMs: number = Date.now()) => {
    if (activeSegmentStartRef.current != null) return;
    activeSegmentStartRef.current = nowMs;
    setSessionMinutesLive(getSessionMinutesLive(nowMs));
  }, [getSessionMinutesLive]);

  const persistReadingSegment = useCallback(async (endedAtMs: number = Date.now()) => {
    pauseActiveSession(endedAtMs);
    ...
    emitDailyGoalReadingEvent({
      userId: user.id,
      minutes: segment.minutes,
      occurredAtIso: segment.endedAtIso,
    });

    const [recordResult, gardenResult, engagementResult] = await Promise.allSettled([
      recordReadingSession({
        user_id: user.id,
        started_at: segment.startedAtIso,
        ended_at: segment.endedAtIso,
        minutes: segment.minutes,
        book_id: bookId,
        book_title: book?.title ?? null,
      }),
      applyGardenProgress(user.id, { readingMinutes: segment.minutes, source: 'reading' }),
      addReadMinutes(user.id, segment.minutes),
    ]);

    if (recordResult.status === 'rejected') {
      logger.error('Failed to persist reading session:', recordResult.reason);
    }
    if (gardenResult.status === 'rejected') {
      logger.warn('Failed to apply garden progress for reading session', gardenResult.reason);
    }
    if (engagementResult.status === 'fulfilled') {
      const engagement = engagementResult.value;
      if (engagement.sessions >= 3 || engagement.minutes >= 20) {
        upgradePrompt.requestShow(user.id, 'read_sessions', { isGuest }).catch(() => {});
      }
    }
  }, [book?.title, bookId, isGuest, pauseActiveSession, upgradePrompt, user]);

  useEffect(() => {
    const syncLiveMinutes = () => setSessionMinutesLive(getSessionMinutesLive());
    syncLiveMinutes();
    const timer = setInterval(syncLiveMinutes, 15 * 1000);
  ...
  useEffect(() => {
    ...
    activeDurationMsRef.current = 0;
    screenActiveDurationMsRef.current = 0;
    activeSegmentStartRef.current = openedAt;
    setSessionMinutesLive(0);
    appStateRef.current = AppState.currentState;

    track('reading_session_started', {
      book_id_hash: hashAnalyticsId(bookId),
      source: 'reader_screen',
    });
    ...
    const appStateSubscription = AppState.addEventListener('change', (nextState) => {
      ...
      if (prevState === 'active' && nextState !== 'active') {
        void flushCurrentReadingProgress();
        void persistReadingSegment();
      }
      if (prevState !== 'active' && nextState === 'active') {
        resumeActiveSession();
      }
    ...
    return () => {
      ...
      const sessionWindow = buildReadingSessionWindow(endedAtMs, screenActiveDurationMsRef.current);

      track('reading_session_ended', {
        book_id_hash: hashAnalyticsId(bookId),
        duration_ms: sessionWindow.durationMs,
        page_turns: pageTurnCountRef.current,
        percent_complete: totalPagesRef.current > 0
          ? Math.round((currentPageRef.current / totalPagesRef.current) * 100)
          : undefined,
        source: 'reader_screen',
      });
      track('session_ended', {
      ...
      void persistReadingSegment(endedAtMs);
    };
  }, [bookId, flushCurrentReadingProgress, pauseActiveSession, persistReadingSegment, resumeActiveSession, user]);
  ...
  const loadReadingProgress = useCallback(async () => {
    ...
    if (local?.cfi) {
      ...
      latestPersistableCfiRef.current = local.cfi;
      ...
      if (hasTotal && typeof local.totalLocations === 'number') {
        const totalFromCache = Math.floor(local.totalLocations);
        cachedBookTotalPagesRef.current = totalFromCache;
        setTotalPages(bookId, totalFromCache);
      }
      ...
      if (hasIndex) {
        ...
        setCurrentPage(bookId, Math.floor(local.locationIndex0 as number) + 1);
        setHasResolvedCurrentPage(true);
        // If we also know total locations, we can hide the loading spinner.
        setPageLoading(bookId, !hasTotal);
      } else {
        cachedBookTotalPagesRef.current = 0;
        setCurrentPage(bookId, 0);
        setHasResolvedCurrentPage(false);
        setPageLoading(bookId, true);
      }
    }
    if (!local?.cfi) {
      cachedBookTotalPagesRef.current = 0;
      ...
      setCurrentPage(bookId, 0);
      setTotalPages(bookId, 0);
      setHasResolvedCurrentPage(false);
      setPageLoading(bookId, true);
    }

    const waitForRemoteResume = shouldDeferReaderMountForRemote({
      hasLocalCfi: Boolean(local?.cfi),
      hasUser: Boolean(user?.id),
      readingProgressEnabled,
    });

    // Mount immediately unless remote progress could still override an initial cover-page location.
    if (!waitForRemoteResume) {
    ...
    try {
      ...
      if (remoteCfi) {
        ...
        if (shouldUseRemote) {
          const remoteChanged = local?.cfi ? local.cfi !== remoteCfi : true;
          ...
          if (remoteChanged) {
            // Prevent stale local page index from showing after account merge/import.
            cachedBookTotalPagesRef.current = 0;
            setCurrentPage(bookId, 0);
            setHasResolvedCurrentPage(false);
            setPageLoading(bookId, true);
          ...
          latestPersistableCfiRef.current = remoteCfi;
          void setCachedLastCfi(
            userId,
            bookId,
            remoteCfi,
            remoteUpdatedAt > 0 ? remoteUpdatedAt : undefined,
            { preservePosition: !remoteChanged }
          );
  ...
  }, [bookId, goToLocation, highlightsEnabled, readingProgressEnabled, user]);
  ...
  const loadListsForSave = useCallback(async () => {
    ...
    try {
      ...
      if (lists.length === 0) {
        const created = await createVocabList(user.id, 'My Words', { maxLists: studyListCap });
      ...
      const lockedIds = getLockedStudyListIds(sorted, isPremium, FREE_STUDY_LIST_CAP);
      const accessible = sorted.filter((list) => !lockedIds.has(list.id));
      setVocabLists(sorted);

      const recent = await fetchMostRecentVocabList(user.id);
      if (recent?.id && !lockedIds.has(recent.id)) {
        setSelectedListId(recent.id);
        return;
      }

      if (accessible.length > 0) {
        setSelectedListId(accessible[0].id);
        return;
      }
  ...
  useEffect(() => {
    if (!selectedListId) {
      if (accessibleVocabLists.length > 0) {
        setSelectedListId(accessibleVocabLists[0].id);
      }
      return;
    }

    if (!lockedVocabListIds.has(selectedListId)) return;
    if (accessibleVocabLists.length > 0) {
      setSelectedListId(accessibleVocabLists[0].id);
      return;
    }
    setSelectedListId(null);
  }, [accessibleVocabLists, lockedVocabListIds, selectedListId]);
  ...
  useEffect(() => {
    ...
    return unsubscribe;
  }, [flushCurrentReadingProgress, navigation]);
  ...
  const loadBookData = async () => {
    try {
      ...
      Alert.alert(t('common.error'), t('reader.bookLoadFailed'));
    }
  };
  ...
  const handleTranslate = useCallback(async () => {
    ...
    track('translate_opened', {
      book_id_hash: hashAnalyticsId(book.id),
      word_length: selection.text.length,
      source: 'reader_screen',
      placement: 'selection_toolbar',
    });
    setTranslating(true);
  ...
  const handleSaveStudyWord = useCallback(async () => {
    ...
    if (lockedVocabListIds.has(selectedListId)) {
      setSnackbar({ visible: true, message: getStudyListLimitMessage(studyListCap), type: 'error' });
      return;
    }
    ...
    try {
      ...
      track('vocab_saved', {
        book_id_hash: hashAnalyticsId(book.id),
        list_id_hash: hashAnalyticsId(selectedListId),
        book_id: book.id,
        word: selection.text,
      ...
      applyGardenProgress(user.id, { savedCount: 1, source: 'saved' }).catch(() => {});
  ...
  }, [addAnnotation, book, clearWebViewSelection, countOverlappingHighlights, currentPage, highlightsEnabled, isGuest, lockedVocabListIds, sameLanguage, selectedListId, selection, studyListCap, studyStore, t, targetLang, translation, upgradePrompt, user]);
  ...
  const handleCreateNewList = useCallback(async (listName: string) => {
    if (!user) return;
    const eligibility = await checkStudyListCreationEligibility(user.id, vocabLists.length, isPremium);
    if (!eligibility.ok) {
      setSnackbar({ visible: true, message: eligibility.message, type: 'error' });
      return;
    }
    try {
      const newList = await createVocabList(user.id, listName, { maxLists: studyListCap });
      await recordStudyListCreated(user.id);
      setVocabLists((prev) => sortStudyListsByRecentUpdate([...prev, newList]));
  ...
  }, [isPremium, studyListCap, t, user, vocabLists.length]);
  ...
  const handleTapLeftEdge = useCallback(() => {
    ...
    if (totalPages > 0) setPageLoading(bookId, false);
    goPrevious?.();
  }, [atStart, bookId, goPrevious, navigationMode, selection, setPageLoading, showTranslateSheet, totalPages]);
  ...
  const handleTapRightEdge = useCallback(() => {
    ...
    if (totalPages > 0) setPageLoading(bookId, false);
    goNext?.();
  }, [atEnd, bookId, goNext, navigationMode, selection, setPageLoading, showTranslateSheet, totalPages]);
  ...
  const handleReaderError = useCallback((error: string) => {
    ...
    track('api_error', {
      endpoint: 'reader_display',
      code: error || 'display_error',
      source: 'reader_screen',
    });
  ...
  const handleReaderReady = useCallback((readyTotalLocations: number, readyLocation: Location, readyProgress: number) => {
    ...
    const readyPage = resolveReadyPage({
      locationIndex: idx,
      hasInitialLocation: Boolean(initialLocation),
      resumeConfirmed,
      fallbackCurrentPage: useReaderStore.getState().progressByBook[bookId]?.currentPage ?? 0,
    });
    readyBookIdRef.current = bookId;

    const knownTotalPages = cachedBookTotalPagesRef.current > 0 ? cachedBookTotalPagesRef.current : 0;
    setCurrentPage(bookId, readyPage);
    setPageLoading(bookId, readyPage <= 0 || knownTotalPages <= 0);
    if (typeof displayedPage === 'number' && typeof displayedTotal === 'number' && displayedTotal > 0) {
      setChapterDisplayed(bookId, displayedPage, displayedTotal);
    }
    if (readyLocation?.start?.href) {
      setCurrentSectionHref(readyLocation.start.href);
    }
    if (cfi && (!initialLocation || resumeConfirmed || isSameCfiLocation(cfi, initialLocation))) {
  ...
  }, [bookId, initialLocation, resumeConfirmed, scheduleSaveReadingProgress, setChapterDisplayed, setPageLoading]);
  ...
  const handleLocationChange = useCallback((
    ...
    const shouldPersistLocation = !initialLocation || resumeConfirmed || isSameCfiLocation(cfi, initialLocation);

    if (waitingForResumeLocationRef.current) {
      const resumeMatched = isSameCfiLocation(cfi, initialLocation);
      if (!resumeMatched) {
        logger.info('Ignoring pre-resume relocation while waiting for initial CFI', {
          cfi,
          expected: initialLocation,
        });
        return;
      }
      waitingForResumeLocationRef.current = false;
    ...
    if (locDisplayed && typeof locDisplayed.page === 'number' && typeof locDisplayed.total === 'number' && locDisplayed.total > 0) {
      setChapterDisplayed(bookId, locDisplayed.page, locDisplayed.total);
      const now = Date.now();
      const times = pageTurnTimesRef.current;
    ...
    const isResumePending = Boolean(initialLocation) && !resumeConfirmedRef.current;
    const expectedFallbackIndex =
      isResumePending && typeof resumeGuardRef.current?.expectedIndex0 === 'number'
        ? resumeGuardRef.current.expectedIndex0
        : null;
    // Update page counter from the reader's generated location index (0-based).
    const idx0 = resolveLocationIndex0({
      locationIndex: loc?.start?.location,
      progress,
      totalLocations: totalLocs,
      allowProgressFallback: !isResumePending,
      fallbackIndex0: expectedFallbackIndex,
    });
    const hasTotalLocs = typeof totalLocs === 'number' && Number.isFinite(totalLocs) && totalLocs > 0;
    if (hasTotalLocs) {
      const nextTotalPages = Math.floor(totalLocs);
      const storeTotalPages = useReaderStore.getState().progressByBook[bookId]?.totalPages ?? 0;
      if (nextTotalPages !== storeTotalPages) {
        setTotalPages(bookId, nextTotalPages);
      }
    }
    ...
    if (guard && Date.now() < guard.expiresAt && guard.expectedIndex0 !== null) {
      ...
      const idxLooksTooEarly = idx0 != null && idx0 < Math.max(0, expected - 2);
      const cfiMatchesExpected = cfi ? isSameCfiLocation(cfi, guard.expectedCfi) : false;
      if (idxLooksTooEarly && !cfiMatchesExpected) {
        return;
      }
      if (idx0 == null && !cfiMatchesExpected) {
        return;
      }
      if (cfiMatchesExpected || (idx0 != null && idx0 >= Math.max(0, expected - 2))) {
        resumeGuardRef.current = null;
      }
    }
    ...
    if (idx0 != null) {
      ...
      const storePage = useReaderStore.getState().progressByBook[bookId]?.currentPage ?? 0;
      if (newPage !== storePage) {
        pageTurnCountRef.current += 1;
        track('page_turned', {
          book_id_hash: hashAnalyticsId(bookId),
          current_page: newPage,
          total_pages: hasTotalLocs ? Math.floor(totalLocs) : totalPagesRef.current,
          source: 'reader_screen',
        });
        setCurrentPage(bookId, newPage);
      }
      setPageLoading(bookId, !hasTotalLocs);
      setChapterLeftPct(bookId, null);

      // Persist numeric index only when locations are ready (avoid clobbering cache with idx0=0).
      if (hasTotalLocs && cfi && shouldPersistLocation) {
  ...
  }, [bookId, currentPage, initialLocation, resumeConfirmed, scheduleSaveReadingProgress, setChapterDisplayed, setChapterLeftPct, setCurrentPage, setPageLoading, setTotalPages, user?.id]);
  ...
  useEffect(() => {
    if (!progressLoaded || initialLocation) return;
    waitingForResumeLocationRef.current = false;
    if (resumeFallbackTimerRef.current) clearTimeout(resumeFallbackTimerRef.current);
    resumeFallbackTimerRef.current = null;
    setResumeConfirmed(true);
  ...
  useEffect(() => {
    ...
    resumeRetryTimerRef.current = null;
    if (resumeFallbackTimerRef.current) clearTimeout(resumeFallbackTimerRef.current);
    const retryDelaysMs = [120, 620, 1400];
    const scheduleRetry = (attemptIdx: number) => {
      if (attemptIdx >= retryDelaysMs.length) return;
      resumeRetryTimerRef.current = setTimeout(() => {
        if (!waitingForResumeLocationRef.current) return;
        try {
          goToLocation(initialLocation);
        } catch {
          // best-effort; next retry/fallback handles it
        }
        scheduleRetry(attemptIdx + 1);
      }, retryDelaysMs[attemptIdx]);
    };

    scheduleRetry(0);
    resumeFallbackTimerRef.current = setTimeout(() => {
      if (!waitingForResumeLocationRef.current || resumeConfirmedRef.current) {
        resumeFallbackTimerRef.current = null;
        return;
      }
      try {
        ...
        // ignore and proceed to timeout release below
      }
      setTimeout(() => {
        if (!waitingForResumeLocationRef.current || resumeConfirmedRef.current) return;
        waitingForResumeLocationRef.current = false;
        setResumeConfirmed(true);
        resumeConfirmedRef.current = true;
      }, 900);
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <ReaderEdgeTapOverlay onTapLeft={handleTapLeftEdge} onTapRight={handleTapRightEdge}>
          ...
          <Reader
            ...
            onLocationsReady={(_, locs) => {
              if (readyBookIdRef.current !== bookId) return;
              if (Array.isArray(locs) && locs.length > 0) {
                const nextTotal = Math.floor(locs.length);
                cachedBookTotalPagesRef.current = nextTotal;
                setTotalPages(bookId, nextTotal);
                void setCachedTotalLocations(user?.id ?? null, bookId, nextTotal, lastSavedCfi.current ?? undefined);

                const storePage = useReaderStore.getState().progressByBook[bookId]?.currentPage ?? 0;
                const idxFromReader = currentLocation?.start?.location;
                const nextPage = resolveLocationsReadyPage({
                  storePage,
                  locationIndex: idxFromReader,
                  hasInitialLocation: Boolean(initialLocation),
                });
                if (nextPage > 0 && storePage <= 0) {
                  setCurrentPage(bookId, nextPage);
                }
                setPageLoading(bookId, nextPage <= 0 || nextTotal <= 0);
              }
            }}
        ...
        {!navigationMode && (
          <ReaderOverlays
            ...
            dailyMinutesRead={readerMinutesToday}
      ...
      <TranslateSheet
        ...
        selectedListName={accessibleVocabLists.find((l) => l.id === selectedListId)?.name ?? null}
        onSelectList={() => {
          logger.info('Opening vocab list picker', {
            listCount: accessibleVocabLists.length,
            selectedListId,
          });
          setListPickerVisible(true);
        }}
        listPickerVisible={listPickerVisible}
        lists={accessibleVocabLists}
        ...
        maxListsReached={vocabLists.length >= studyListCap}
        limitReachedLabel={getStudyListLimitMessage(studyListCap)}
