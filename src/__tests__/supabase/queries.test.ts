...
describe('user settings functions', () => {
  it('exports user settings functions', () => {
    ...
    expect(typeof q.syncUserPremiumEntitlement).toBe('function');
  });
});
...
describe('account functions', () => {
  it('exports delete/reactivation helpers', () => {
    const q = require('@/supabase/queries');
    expect(typeof q.deleteUserAccount).toBe('function');
    expect(typeof q.isUserSoftDeleted).toBe('function');
    expect(typeof q.reactivateUserAccount).toBe('function');
  });
});
